...
void main_task(intptr_t unused)
{
    ...
    ev3_lcd_draw_string("EV3way-ET sample_cpp\n", 0, CALIB_FONT_HEIGHT*1);

    int16_t    sGyroValue = 0;
    GYRO_STATE GyroState  = GSTA_UNSTABLE;
    char cString[50];
    ...
    while( true ) {
        ...
        GyroState  = gGyroAdmin->getState();
        sGyroValue = gGyroAdmin->getValue();

        sprintf( cString, "[sGyroValue =  %5d]\n \0", sGyroValue );
        ev3_lcd_draw_string( cString, 0, CALIB_FONT_HEIGHT*2 );

        switch( GyroState ) {
            case GSTA_UNSTABLE:
            ev3_lcd_draw_string("GSTA_UNSTABLE\n", 0, CALIB_FONT_HEIGHT*3 );
            break;

            case GSTA_STABILITY:
            ev3_lcd_draw_string("GSTA_STABILITY\n", 0, CALIB_FONT_HEIGHT*3 );
            break;

            case GSTA_FALLING:
            ev3_lcd_draw_string("GSTA_FALLING\n", 0, CALIB_FONT_HEIGHT*3 );
            goto FALLING;
            break;

            default:
            ev3_lcd_draw_string("err\n", 0, CALIB_FONT_HEIGHT*3 );
            break;
        }
