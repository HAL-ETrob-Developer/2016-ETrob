...
void main_task(intptr_t unused)
{
    ...
    SENC_CLR SencClr = SCLR_BLACK;
    
    while( true ) {
        tail_control(TAIL_ANGLE_DRIVE); /* バランス走行用角度に制御 */
        
        gRayReflectAdmin->callValueUpDate();
        Value = gRayReflectAdmin->getValue();
        SencClr = gRayReflectAdmin->getState();
        
        sprintf(( char* )cString, "ColorValue[%3d]\n",Value);

        ev3_lcd_draw_string( cString, 0, CALIB_FONT_HEIGHT*2);

        switch( SencClr ) {
            case SCLR_BLACK:
                ev3_lcd_draw_string( "SCLR_BLACK", 0, CALIB_FONT_HEIGHT*3 );
                break;

            case SCLR_GRAY:
                ev3_lcd_draw_string( "SCLR_GRAY ", 0, CALIB_FONT_HEIGHT*3 );
                break;

            case SCLR_WHITE:
                ev3_lcd_draw_string( "SCLR_WHITE", 0, CALIB_FONT_HEIGHT*3 );
                break;

            default:
                ev3_lcd_draw_string( "err", 0, CALIB_FONT_HEIGHT*3);
                break;
        }
